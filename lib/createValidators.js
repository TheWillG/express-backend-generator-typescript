...
const createValidators = async (resourceName, resource) => {
  const actions = ['get', 'put', 'post', 'delete'];
  const validatorsDir = './generatedApp/src/controllers/validators';
  ...
  let exportsTo = '';
  const validatorFolderPath = `./generatedApp/src/controllers/validators/${resourceName}`;
  const files = `./generatedApp/src/controllers/validators/${resourceName}/index.ts`;
  ...
  try {
    ...
    await replace(validatorsOptions);
